...
void addOption(char* text, subMenus parentSub, void(*f)()) {
	if (parentSub == menu.subMenu) {
		...
		if (menu.scroll == menu.options) { //current scroll
			...
			drawText(text, menu.x + menu.optionPadding, menu.y + menu.height + (menu.optionSpacing * (menu.options)) + menu.optionSpacing * 0.5, menu.font, menu.optionFontSize, menu.colors.scrollText, 0, 0.5);
		}
		else {
